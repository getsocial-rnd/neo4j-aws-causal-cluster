...
RUN apt update && apt install -y --no-install-recommends \
	...
	jq python lsof host \
	logrotate && \ 
	pip install awscli  && apt remove -y python-pip  python-setuptools

# Install plugins
RUN mkdir -p /var/lib/neo4j/plugins

ENV NEO4J_APOC_VERSION=3.5.0.4
...
RUN curl -L -s https://github.com/neo4j-contrib/neo4j-apoc-procedures/releases/download/$NEO4J_APOC_VERSION/apoc-$NEO4J_APOC_VERSION-all.jar \
	...
	curl -L -s http://central.maven.org/maven2/mysql/mysql-connector-java/6.0.6/mysql-connector-java-6.0.6.jar \
